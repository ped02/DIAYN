...
class HRLPolicy(torch.nn.Module):
    ...
    def get_loss(self, states, actions, next_states):
        current_skill = self.skill_selector(states)
        next_skill = self.skill_selector(next_states)

        return self.smoothness_loss_factor * torch.mean(
            (next_skill - current_skill) ** 2
        )
...
if __name__ == '__main__':
    ...
    with open(current_dir + '/config/ur5e_config_hrl_6.yaml', 'r') as file:
