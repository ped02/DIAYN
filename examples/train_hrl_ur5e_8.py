...
class HRLPolicy(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.skill_selector = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(128, 2 * skill_dim),
        )
    ...
    def forward(self, states):
        skill_mean, skill_log_std = self.skill_selector(states).chunk(2, dim=-1)
        skill_mean = torch.softmax(skill_mean, dim=-1)
        return torch.concatenate([skill_mean, skill_log_std], dim=-1)
...
def main(
    ...
    def transform_action(states, actions):
        return skill_policy.get_action(
            torch.concatenate([states, actions], axis=-1), noisy=False
        )
...
if __name__ == '__main__':
    ...
    with open(current_dir + '/config/ur5e_config_hrl_8.yaml', 'r') as file:
