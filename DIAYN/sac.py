...
class SAC:
    def __init__(self, 
        ...
        self.action_low = torch.tensor(action_low, device=self.device) # [action_dim]
        ...
        self.policy = policy_class(state_dim, action_dim, **policy_kwargs).to(device)
        self.q1 = q_network_class(state_dim, action_dim, **q_network_kwargs).to(device)
        self.q2 = q_network_class(state_dim, action_dim, **q_network_kwargs).to(device)
        ...
        # Optimizers
        self.policy_optimizer = torch.optim.Adam(self.policy.parameters(), **policy_optimizer_kwargs)
        self.q_optimizer = torch.optim.Adam([*self.q1.parameters(), *self.q2.parameters()], **q_optimizer_kwargs)
    ...
    def set_state_dict(self, state_dict: Mapping[str, Any]):
        ...
        self.policy_target.load_state_dict(state_dict['policy_target_state_dict'])
        ...
        if state_dict.get('action_low') is not None:
            self.action_low = torch.tensor(state_dict['action_low'], dtype=torch.float32, device=self.device)
    ...
    def get_q_loss(self, states, actions, rewards, next_states, not_dones, gamma=0.99):
        ...
        q_loss = torch.mean((current_q1 -  q_target) ** 2 + (current_q2 -  q_target) ** 2)
