...
import DIAYN.envs
from DIAYN.utils import replay_post_processor, pad_to_dim_2, plot_to_image, image_numpy_to_torch
...
def plot_skill_trajectories(
    ...
    for z in range(num_skills):
        skill_index = torch.tensor(z).reshape(1,)
        ...
        for t in range(num_trajectories):
            ...
            for step in range(steps_per_episode):
                ...
                with torch.no_grad():
                    actions = agent.get_action(observations.to(agent.device), noisy=True).cpu()
                next_observations_raw, rewards_raw, terminated, truncated, _ = env.step(actions.numpy())

                next_observations = torch.cat([pad_to_dim_2(torch.Tensor(next_observations_raw)), expanded_skill_vector], dim=-1)

                with torch.no_grad():
                    discriminator_logits = agent.discriminator(pad_to_dim_2(torch.Tensor(next_observations_raw)).to(agent.device))
                psuedo_reward = torch.nn.functional.log_softmax(discriminator_logits, dim=-1).gather(-1, skill_index.unsqueeze(0).to(agent.device)) + torch.log(torch.tensor(agent.skill_dim, device=agent.device)) # Assume uniform distribution

                observations = next_observations
                
                # sample.append(next_observations_raw.tolist())
                sample.append([*next_observations_raw.tolist(), psuedo_reward.item()])
    ...
    for skill_id in range(num_skills):
        for traj_id in range(num_trajectories):  # Iterate over trajectories
            trajectory = skills_trajectories_np[skill_id, traj_id]  # Extract (16, 2) trajectory
            plt.plot(trajectory[:, 0], trajectory[:, 1], color=colors(skill_id), label=f"Skill {skill_id}" if traj_id == 0 else "")
...
def plot_phase_skill(
    ...
    colors = plt.get_cmap("tab20c", num_skills)  # Use categorical colormap with exactly k colors
    cmap = ListedColormap([colors(i) for i in range(num_skills)])  # Ensure colors are fixed
    ...
    with torch.no_grad():
        result = agent.discriminator(torch.Tensor(grid).to(agent.device).reshape(-1,2)).cpu().numpy()

    skill_classification = np.argmax(result, axis=-1)
    skill_grid = skill_classification.reshape(grid.shape[:-1])
    fig = plt.figure()
    plt.imshow(skill_grid, extent=[-3.0, 3.0, -3.0, 3.0], origin='lower', cmap=cmap, norm=norm)
...
def main(
    ...
    # Setup env variables
    envs = gym.make_vec(
        environment_name, vectorization_mode=gym.VectorizeMode.SYNC, num_envs=num_envs
    )
    ...
    replay_buffer = ReplayBuffer(replay_buffer_size, post_processor=lambda x:replay_post_processor(x, device))
    ...
    def plot_skill_trajectories_phase():
        skill_trajectory_visual = plot_skill_trajectories(environment_name, plot_trajectories, num_steps, diayn_agent, plot_dpi)
        phase_skill_visual = plot_phase_skill(num_skills, diayn_agent, dpi=plot_dpi)

        log_writer.add_image('Skill/Trajectory', image_numpy_to_torch(skill_trajectory_visual), total_steps)
        log_writer.add_image('Skill/Phase', image_numpy_to_torch(phase_skill_visual), total_steps)

    def training_function(step):
        nonlocal total_steps
        if len(replay_buffer.buffer) > num_steps * num_skills * num_envs * 10: # at least 10 demonstrations of each randomly before start training
            diayn_agent.update(
                replay_buffer,
                step=total_steps,
    ...
    for episode in range(episodes):
        if (episode + 1) % 200 == 0:
            print(f'Starting {episode + 1} / {episodes} @ {time.time():.3f} sec')
        skill_index = torch.randint(0, num_skills, (1,))
        skill_vector = torch.nn.functional.one_hot(skill_index, num_classes=num_skills)
        mean_step_reward = rollout_skill(envs, 
...
if __name__ == '__main__':
    ...
    log_path = 'runs/diayn_2d_2'

    main(environment_name, num_envs, episodes, num_steps, num_skills, log_path=log_path)
